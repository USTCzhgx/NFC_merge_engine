...
module fcc_executer #(
    ...
    output     [7:0]               o_sr_r,
    // Command done pulse (1-cycle) for accepted command
...
wire           o1_done;  // erase done
wire           o2_done;  // read done
wire           o3_done;  // prog done
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        o_cmd_done <= 1'b0; // default pulse

        // Accept a new external command
        if (i_cmd_valid && o_cmd_ready) begin
            inflight       <= 1'b1;
            inflight_type  <= i_cmd_type;
            pending_status <= 1'b0;
        end

        // If prog/erase requests an internal read-status, mark pending_status
        // (read-status completion is indicated by o_cmd_ack from phy_status)
        if (inflight && (o1_rd_st_req || o3_rd_st_req)) begin
            pending_status <= 1'b1;
        end

        // Final done condition
        if (inflight) begin
            if (pending_status) begin
                // Command is only complete after phy_status acks
                if (o_cmd_ack) begin
                    o_cmd_done     <= 1'b1;
                    inflight       <= 1'b0;
                    pending_status <= 1'b0;
                ...
                // No extra status needed: done comes from the active phy
                case (inflight_type)
                    2'b00: begin // status command itself
                        if (o_cmd_ack) begin
                            o_cmd_done <= 1'b1;
                            inflight   <= 1'b0;
                        end
                    end
                    2'b01: begin // phy_cmd path (you wired to phy_erase)
                        if (o1_done) begin
                            o_cmd_done <= 1'b1;
                            inflight   <= 1'b0;
                        end
                    end
                    2'b10: begin // phy_read
                        if (o2_done) begin
                            o_cmd_done <= 1'b1;
                            inflight   <= 1'b0;
                        end
                    end
                    2'b11: begin // phy_prog
                        if (o3_done) begin
                            o_cmd_done <= 1'b1;
                            inflight   <= 1'b0;
                        end
